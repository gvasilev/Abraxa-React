            ...
            },
            {
                ...
                handler: function (me) {
                    let store = me.upVM().get('disbursements'),
                        ...
                        allDisbursmentsFromGroup = store.query('group_id', disbursement.get('group_id'));
                    Ext.Msg.confirm(
                        'Delete',
                        'Are you sure you want to delete this item?',
                        function (answer) {
                            if (answer === 'yes') {
                                if (allDisbursmentsFromGroup.items && allDisbursmentsFromGroup.items.length == 1) {
                                ...
                                if (allDisbursmentsFromGroup.items.length) {
                                    Ext.Array.each(allDisbursmentsFromGroup.items, function (rec) {
                                        rec.set(disbursement.get('type') + '_id', null);
                                    });
                                }
                                let disbursementExpenses = expenses.query(
                                    disbursement.get('type') + '_id',
                                    disbursement.get('id')
                                );
                                ...
                                store.sync({
                                    success: function () {
                                        me.upVM().get('selectedAccount').load();
