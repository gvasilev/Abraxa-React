Ext.define('Abraxa.view.portcall.accounts.AccountCreate', {
    ...
    viewModel: {
        ...
        formulas: {
            costCenterFilter: {
                bind: {
                    bindTo: '{billToCombo.selection}',
                    deep: true,
                },
                get: function (billTo) {
                    let store = this.get('sharedCostCenter');

                    if (store) store.clearFilter();

                    if (billTo && billTo.get('is_tenant')) {
                        return function (rec) {
                            return rec.get('company_id') === billTo.get('is_tenant').id;
                        };
                    } else {
    ...
    items: [
        {
            ...
            items: [
                ...
                {
                    xtype: 'combobox',
                    testId: 'portcallAccountCreateCustomerAccountingCodesCombo',
                    labelAlign: 'left',
                    ...
                    hidden: true,
                    queryMode: 'local',
                    itemTpl: '<div class="a-combo-item">{name}</div>',
                    valueField: 'id',
                    displayField: 'name',
                    bind: {
                        hidden: '{accountAdvanced ? false : true}',
                        value: '{record.custom_cost_center}',
                        store: '{sharedCostCenter}',
                        placeholder: '{sharedCostCenter.count ? "Choose cost center" : "No cost centers available"}',
                    },
                },
                {
                    xtype: 'button',
