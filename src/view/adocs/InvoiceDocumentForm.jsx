                ...
                },
                {
                    ...
                    listeners: {
                        select: function (cmp, selection) {
                            const invoiceFormVM = cmp.upVM();
                            const selectOrg = selection.get(AbraxaConstants.labels.organization);

                            const address = selectOrg && selectOrg.org_address ? selectOrg.org_address : '';
                            const address2 = selectOrg && selectOrg.org_address_2 ? selectOrg.org_address_2 : '';
                            const memo = selectOrg && selectOrg.org_memo ? selectOrg.org_memo : '';

                            const city = selectOrg && selectOrg.city_name ? selectOrg.city_name : '';
                            const country = selectOrg && selectOrg.country_name ? selectOrg.country_name : '';
                            const orgNameLabel = AbraxaConstants.labels.organizationName;
                            const orgName = selection.get(orgNameLabel) ? selection.get(orgNameLabel) : '';

                            invoiceFormVM.set('document_data.bill_to_address', address);
                            invoiceFormVM.set('document_data.bill_to_address_2', address2);
                            invoiceFormVM.set('document_data.bill_to_memo', memo);

                            invoiceFormVM.set('document_data.bill_to_city', city);
                            invoiceFormVM.set('document_data.bill_to_country', country);
                            invoiceFormVM.set('document_data.bill_to_name', orgName);
                        },
                    },
                    required: true,
                    disabled: false,
        ...
        items: [
            ...
            },
            {
                ...
                handler: function (btn) {
                    const form = btn.up('dialog').down('formpanel');

                    if (form.validate()) {
                        btn.up('dialog').setMasked({
                            xtype: 'loadmask',
                            style: {
                                zIndex: 9999,
                            },
                        ...
                        btn.up('dialog').getController().generateFinancialPdfDocument(btn);
                    } else {
