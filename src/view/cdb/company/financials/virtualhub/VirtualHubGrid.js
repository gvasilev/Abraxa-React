Ext.define('Abraxa.view.cdb.company.financials.virtualhub.VirtualHubGrid', {
    extend: 'Ext.grid.Grid',
    xtype: 'company.virtualhub.grid',
    testId: 'compVirtualHubGrid',
    flex: 1,
    cls: 'a-disbursements-grid a-wps-financial-grid abraxa-grid',
    ui: 'bordered',
    stateful: ['plugins', 'columns'],
    stateId: 'company-virtualhub-grid-state',
    itemId: 'company-virtualhub-grid',
    reference: 'companyVirtualHubGrid',
    infinite: false,
    store: [],
    collapsible: false,
    bind: {
        store: '{virtualHubPayments}',
    },
                ...
                listeners: {
                    initialize: function () {
                        this.add({
                            ...
                            cls: 'sm-title',
                            bind: {
                                html: '<strong>{totalVirtualHubRecords}</strong> records',
                            },
                        });
                        this.add({
                            xtype: 'div',
                            testId: 'compVirtualHubGridNoRecordsDiv',
                            width: '60%',
                            bind: {
                                html: '<div class="h5">Total open balance</div><div class="a-billed-price"><span class="a-billed-currency">{currentCompany.default_currency}</span><span class="a-billed-amount">{totalBalance}</span></div>',
                            },
                        });
                    },
                },
    ...
    columns: [
        ...
        },
        {
            ...
            cell: {
                ...
                encodeHtml: false,
                bind: {
                    tpl: '<a data-id="vessel_name" class="a_grid_action text-truncate a_vessel_name a-vessel-name" href="#portcall/{record.owner.id}" title="{record.owner.voyage.vessel_name}">{record.owner.voyage.vessel_name}</a><div class="sm-title">{record.owner.file_id ? record.owner.file_id : "---"}</div>',
                },
                listeners: {
                    click: {
                        element: 'element',
        ...
        },
        {
            ...
            cell: {
                ...
                cls: 'a-cell-amount',
                bind: {
                    tpl: '{record.account.balance:number("0,000.00")}',
                },
            },
            filterType: {
                // required configs
