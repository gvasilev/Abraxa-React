                        ...
                        if (editMode) {
                            ...
                            store.sync({
                                success: function (batch, opt) {
                                    ...
                                    // NOTE: portserveRecord is null (couldn't find an isntance when it isn't);
                                    // port_id is in record = vm.get('terminal');
                                    // Also, it is in the store.getProxy().getExtraParams() already.
                                    // To avoid bugs, I am leaving the old code with a check for portserveRecord.
                                    // Needs to be removed/refactored?
                                    if (portserveRecord && portserveRecord.get('port_id')) {
