import './PriceBookService.jsx';
import './PriceBookServiceFormulaBlockRule.jsx';
Ext.define('Abraxa.model.calculator.PriceBookServiceFormulaBlock', {
    extend: 'Ext.data.Model',

    fields: [{ name: 'formula', type: 'string', critical: true }],

    belongsTo: {
        model: 'Abraxa.model.calculator.PriceBookService',
        name: 'priceBookService',
        associationKey: 'price_book_service_id',
    },

    hasMany: [
        {
            model: 'Abraxa.model.calculator.PriceBookServiceFormulaBlockRule',
            name: 'rules',
            associationKey: 'rules',
        },
    ],
});
